  ...
  <div class="min-h-screen pb-20 lg:pb-0">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 lg:py-12">
      ...
      <div class="mb-6 lg:mb-10">
        <h1
          class="text-3xl lg:text-5xl font-display font-light tracking-tight"
          :style="{ color: fg }"
        >
          Earnings
        </h1>
      ...
      >
        ...
        <h2
          class="text-xl lg:text-2xl font-display font-light mb-3"
          :style="{ color: fg }"
        >
          {{
            hasBank
        ...
        <p
          class="text-sm lg:text-base mb-6 max-w-md mx-auto"
          :style="{ color: fg2 }"
        >
          <template v-if="hasBank">
            Your bank account is verified. Ads playing on your screen are
            crediting your account. Payouts run on the 1st of every month (or
            your selected cadence)
          </template>
          <template v-else>
            Each ad that plays on your screen credits your account. Payouts run
            on the 1st of every month, with 5% withholding tax deducted.
          </template>
        </p>
        ...
        >
          <div
            class="text-[11px] uppercase tracking-wider font-semibold mb-3"
            :style="{ color: fg3 }"
          >
            Payout flow
          </div>
          <ol class="text-sm space-y-2" :style="{ color: fg }">
            <li>
              <strong>1.</strong> Ad plays on your screen → revenue is credited.
            </li>
            <li>
              <strong>2.</strong> On the 1st of the month, balance is checked.
            </li>
            <li>
              <strong>3.</strong> Once over Cadence(weekly or monthly), payout
              is created and sent to your primary bank account.
            </li>
            <li>
              <strong>4.</strong> Net amount (gross − 5% WHT) lands in your bank
      ...
      <div v-else class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
        ...
        >
          <div
            class="text-xs lg:text-sm font-semibold tracking-wider uppercase mb-3"
            :style="{ color: fg3 }"
          >
            Total earned
          </div>
          <div
            class="font-display text-5xl lg:text-6xl font-light tracking-tight mb-2"
            :style="{ color: fg }"
          >
            {{ totalEarned }}
          </div>
        ...
        >
          ...
          <div
            class="font-display text-5xl lg:text-6xl font-light tracking-tight mb-2 text-clay-500"
          >
            {{ upcomingNet }}
          </div>
          <div class="text-sm mb-4" :style="{ color: fg3 }">
            {{ upcomingDate }}
          </div>
          <button
            type="button"
      ...
      >
        ...
        >
          <h3 class="text-lg font-bold mb-2" :style="{ color: fg }">
            Request early payout
          </h3>
          <p class="text-sm mb-4" :style="{ color: fg3 }">
            Pays out your current balance now (subject to ops review). Minimum
            balance is ₦20,000.
          </p>
          <textarea
            v-model="earlyReason"
      ...
      <template v-if="hasEarnings">
        ...
        >
          ...
          >
            5% WHT is deducted from each payout as required by FIRS. You receive
            ₦11,970 from every ₦12,600 earned.
          </div>
        </div>

        <div>
          <h2
            class="text-sm lg:text-base font-semibold tracking-wider uppercase mb-4"
            :style="{ color: fg3 }"
          >
            Payout history
          </h2>
          ...
          >
            <div
              ...
              :style="{
                borderBottom:
                  i < payouts.length - 1 ? `1px solid ${divLine}` : 'none',
              }"
            >
              <div
                class="w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0"
                :style="{
                  background: dark ? 'rgba(43,114,68,0.15)' : '#EEF5F1',
                }"
              >
              ...
              <div class="flex-1">
                ...
                <div class="text-xs lg:text-sm mt-1" :style="{ color: fg3 }">
                  {{ p.date }}<span v-if="p.bankName"> · {{ p.bankName }}</span
                  ><span v-if="p.bankTail"> · ••{{ p.bankTail }}</span>
                </div>
              </div>
              <div>
                <div
                  class="font-display text-xl lg:text-2xl font-medium text-moss-500 text-right"
                >
                  ₦{{ p.amount.toLocaleString() }}
                </div>
                <div class="text-xs text-right" :style="{ color: fg3 }">
                  Paid
                </div>
              </div>
            </div>
...
const divLine = computed(() =>
  props.dark ? "rgba(255,255,255,0.06)" : "#F2EFE9",
);
...
const hasBank = computed(() => {
  ...
  const list = Array.isArray(raw)
    ? raw
    : raw?.bankAccounts || raw?.accounts || raw?.data || [];
...
const upcomingNet = computed(() =>
  formatNaira(
    toNaira(
      upcoming.value.netKobo ?? upcoming.value.net ?? upcoming.value.amountKobo,
    ),
  ),
);
